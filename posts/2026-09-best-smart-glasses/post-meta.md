# Post metadata — Best Smart & AI Glasses of 2026

| Field | Value |
|---|---|
| **Title** | `Best Smart Glasses of 2026: Meta Ray-Ban Display vs Ray-Ban Meta Gen 2, Rokid, Xreal, Even Realities — and the AI-Glasses Exam Scandal` |
| **Short title for the slug** | `Best Smart Glasses 2026` → `/2026/09/best-smart-glasses-2026.html` |
| **Labels** | `Smart Glasses, Buying Guide, Premium, Under $500` |
| **Search Description** (≤155) | `Camera AI glasses vs display glasses vs XR: Meta Ray-Ban Display, Ray-Ban Meta Gen 2, Rokid, Xreal, Viture, Even Realities compared, with the privacy settings to change.` |
| **Cover** | `images/cover.png` |
| **Timing note** | Publish before Sept 23; add a "Meta Connect update" paragraph after Sept 24 (Gen 3 details, Gen 2 price cut). |

## Content facts
* 10,630 words (check_post.py); 10 ranked picks + 5 honorable mentions; 8 brand sections; 14-question FAQ; day-one privacy checklist; travel tips; 8-theme complaint analysis; 12-row decision table.
* Graphics: cover (PNG `<img>`), three-kinds diagram, spec table, privacy checklist, complaint themes, decision flowchart.

## Video embeds
| Where | ID | Title as indexed |
|---|---|---|
| Meta brand | `SzQF5lGBs5U` | The Problem with Meta Ray-Ban Gen 2 (A Brutally HONEST Review) |
| Brand table | `zJWMwJrlvIk` | Ray-Ban Meta Gen 3 Smart Glasses Are Coming in 2026 |
| Pick: Rokid Glasses | `Vtni0Dz35sk` | 테크몽 — AI 안경 끼고 수능 문제 풀어봤습니다 (Techmong exam video) |
| Pick: Rokid Glasses | `LKcnV3xZPaU` | Rokid AI Smart Glasses Review — ChatGPT & Gemini, translation, 12MP camera |
| Picks roundup | `bqAVrKlSkT8` | Top 20 Best Smart AI Glasses of 2026 |
| Tips (privacy) | `CPVymrg2d5g` | How To Enable / Disable 'Hey Meta' On Ray-Ban Meta AI Glasses |
| Tips (Short) | `eNeN6F1_JZ8` | AI 안경 끼고 수능 풀어봤더니..소름 (Techmong Short) |
| Decision guide | `m_5lzvPB55k` | 10 Most Insane Smart Glasses of 2026 (Watch Before Buying Meta Ray-Bans!) |

Spare: `e1Pv2-sK9zk` (10 Best Smart Glasses 2026 on Amazon). No dedicated Meta Ray-Ban Display / Xreal One Pro / Even G2 review videos were confirmed via search; the roundups cover them.

## Image slots
9 SiteStripe slots.
